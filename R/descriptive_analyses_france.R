...
X <- names(dta)
X <- X[!(X %in% c("N.Obs", "Sit_fam_Autre", "Specialites", "Specialites_Autre",
                  "Mode_exerc_Autre", "Type_autre_dipl_Autre", "CLE"))]
fig_descr <- list()
...
f <- paste0("results/descriptive_analyses_", d, ".xlsx")
...
rm(d, f, wb, r, s, mc_var, mc_list, mc, cs, i)

# Export figures
d <- format(Sys.time(), "%Y%m%d")
f <- paste0("results/descriptive_analyses_", d, ".pdf")
